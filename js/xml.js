// js/xml.js — v0.9.120 — ВАЛИДАЦИЯ С ПОДСКАЗКОЙ ОБ ОТКЛЮЧЕНИИ + КОММЕНТАРИИ
...
function generateXML() {
  ...
  // Водяной знак при With Comments
  ...
  if (localStorage.getItem('validateXML') === 'true') {
    ...
    document.querySelectorAll('.node.rng').forEach(node => {
      ...
      if (sItems === 0 && fItems === 0) {
        issues.push(`${L.validationNoItems || 'RNG без предметов в ветках'} (${(chance * 100).toFixed(1)}%)`);
      }
    });
  }

  // Дубликаты identifier (отдельный чекбокс)
  ...
  // АЛЕРТ С ПОДСКАЗКОЙ ОБ ОТКЛЮЧЕНИИ
  if (issues.length > 0) {
